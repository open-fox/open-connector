...
[Connector SDK](https://github.com/oomol-lab/connector-sdk)、MCP 和 HTTP 为 AI Agent 提供 8,000+ 个可直接调用的预置 Action。
...
provider；当前开源 catalog 已适配 800+ 个 provider，后续会随着工程迁移持续把更多集成整理成可维护的开源
...
- [连接 800+ 个 provider，提供 8,000+ 个预置 Action](docs/providers.md)，覆盖 GitHub、Gmail、Notion、
...
OpenConnector 当前支持 800+ 个 provider 和 8,000+ 个预置 Action。下面的 logo 墙只是部分展示，不代表完整 catalog。它选取了一组高识别度的 SaaS、开发者工具、效率应用、数据分析产品和 AI 服务，让用户一眼感受到 OpenConnector 覆盖的范围远不止这些 logo。
